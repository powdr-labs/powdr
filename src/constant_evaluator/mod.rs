...
pub fn generate(analyzed: &Analyzed) -> (HashMap<&str, ConcreteColumn>, DegreeType) {
    let mut degree = None;
    ...
    for (i, (poly, _)) in analyzed.constant_polys_in_source_order().iter().enumerate() {
        if let Some(v) = other_constants.get_mut(poly.absolute_name.as_str()) {
            values.insert(
                poly.absolute_name.as_str(),
                ConcreteColumn::new(i, std::mem::take(v)),
            );
        };
    }
...
mod test {
    ...
    fn map<'a>(
        i: impl IntoIterator<Item = (&'a str, Vec<AbstractNumberType>)>,
    ...
    pub fn test_last() {
        ...
        assert_eq!(
            constants,
            map(vec![("F.LAST", convert(vec![0, 0, 0, 0, 0, 0, 0, 1]))])
        );
    }
    ...
    pub fn test_counter() {
        ...
        assert_eq!(
            constants,
            map(vec![("F.EVEN", convert(vec![-2, 0, 2, 4, 6, 8, 10, 12]))])
        );
    }
    ...
    pub fn test_arrays() {
        ...
        assert_eq!(
            ...
            map(vec![
                ...
                ("F.empty", convert([0i32; 10].to_vec())),
                (
                    "F.ref_other",
