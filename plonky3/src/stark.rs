...
use p3_uni_stark::{
    prove_with_key, verify_with_key, StarkGenericConfig, StarkProvingKey, StarkVerifyingKey,
};

use crate::{
    circuit_builder::PowdrCircuit,
    params::{FieldElementMap, Plonky3Field},
};
...
impl<T: FieldElementMap> Plonky3Prover<T> {
    pub fn setup(&mut self) {
        ...
        let pcs = config.pcs();
        let domain = pcs.natural_domain_for_degree(self.analyzed.degree() as usize);
        ...
        let (fixed_commit, fixed_data) = pcs.commit(evaluations);
