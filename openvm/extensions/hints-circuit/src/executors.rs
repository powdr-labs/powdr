...
impl<F: PrimeField32> PhantomSubExecutor<F> for ReverseBytesSubEx {
    fn phantom_execute(
        ...
        let hint_bytes = bytes.into_iter().rev().map(|b| F::from_canonical_u8(b)).collect();
...
impl<F: PrimeField32> PhantomSubExecutor<F> for K256InverseField10x26SubEx {
    fn phantom_execute(
        ...
        let bytes: [u8; FIELD10X26_BYTES] = unsafe { memory.read::<u8, { FIELD10X26_BYTES }>(RV32_MEMORY_AS, rs1) };
