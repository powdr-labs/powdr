    ...
    T: FieldElement,
{
    ...
    for constraint in &machine.constraints {
        ...
        println!("differenct feilds of quadratic symbolic expression:\n quadratic:{:?}\n,linear{:?}\n,const{:?}", 
            quadratic_symbolic_expr.quadratic,quadratic_symbolic_expr.linear, quadratic_symbolic_expr.constant);
        circuit_builder.evaluate_expression(&quadratic_symbolic_expr);
        println!(
            "plonk circuit in quadratic symbolic expression circuit builder: {}",
            circuit_builder.plonk_circuit
        );
    }
    ...
    T: FieldElement,
{
    ...
    pub fn evaluate_expression(
        ...
        quadratic_result.iter().for_each(|var| {
            self.plonk_circuit.add_gate(Gate {
                q_l: T::ONE,
                a: temp.clone(),
                q_r: T::ONE,
        ...
        if let (Some((poly_a, c_a)), Some((poly_b, c_b))) = (iter.next(), iter.next()) {
            let temp_expr = QuadraticSymbolicExpression {
                quadratic: Vec::new(),
            ...
            if let Some(var) = self.cache.get(&temp_expr) {
                temp = var.clone();
                linear_results.push(temp.clone());
            } else {
                self.plonk_circuit.add_gate(Gate {
        ...
        linear_results.iter().for_each(|var| {
            self.plonk_circuit.add_gate(Gate {
                q_l: T::ONE,
                a: temp.clone(),
                q_r: T::ONE,
...
mod tests {
    ...
    fn test_air_to_plonkish() {
        ...
        assert_eq!(
            format!("{}", build_circuit_from_quadratic_simbolic_expression(&machine, &bus_map)),
            "bus: none, x * y = tmp_1
bus: none, -x = tmp_3
bus: none, x + y = tmp_4
