...
fn execute_fast<F: FieldElement>(
    ...
    let (trace_len, _) = powdr::riscv_executor::execute_fast::<F>(
        &asm,
...
fn autoprecompiles<F: FieldElement>(
    ...
    let (trace_len, label_freq) = powdr::riscv_executor::execute_fast(
        &asm,
        initial_memory,
        pipeline.data_callback().unwrap(),
    ...
    let (trace_len, _) = powdr::riscv_executor::execute_fast(
        &asm,
        initial_memory,
        pipeline.data_callback().unwrap(),
        &powdr::riscv::continuations::bootloader::default_input(&[]),
...
fn execute<F: FieldElement>(
    ...
    if continuations {
        ...
        let (execution, _) = powdr::riscv_executor::execute::<F>(
            &asm,
            &pil,
            fixed,
            powdr::riscv_executor::MemoryState::new(),
