...
use ast::parsed::{
    self, ArrayExpression, BinaryOperator, FunctionDefinition, MatchArm, PilStatement,
    PolynomialName, UnaryOperator,
};
            ...
            PExpression::MatchExpression(scrutinee, arms) => Expression::MatchExpression(
                ...
                arms.into_iter()
                    .map(|MatchArm { pattern, value }| MatchArm {
                        pattern: pattern.map(|n| self.process_expression(n)),
