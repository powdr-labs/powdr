        ...
        Ok(match e {
            ...
            Expression::BlockExpression(_, BlockExpression { statements, expr }) => {
                let (formatted_statements, var_height) = statements.iter().try_fold(
                    (vec![], var_height),
                    |(mut acc, var_height), s| {
                        let (formatted, var_height) = self.format_statement(s, var_height)?;
                        acc.push(formatted);
                        Ok::<_, String>((acc, var_height))
                    },
                )?;
                format!(
                    "{{\n{}\n{}\n}}",
                    formatted_statements.into_iter().format("\n"),
                    expr.as_ref()
    ...
    fn format_statement(
        ...
        Ok(match s {
            StatementInsideBlock::LetStatement(LetStatementInsideBlock { pattern, ty, value }) => {
                ...
                let new_vars = pattern.variables().count();
                let value = self.format_expr(value, var_height + new_vars)?;
                let var_name = "scrutinee__";
                let ty = ty
                    .as_ref()
