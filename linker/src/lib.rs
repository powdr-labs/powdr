...
pol commit instr_jmpz_pc_update;
instr_jmpz_pc_update = (XIsZero * instr_jmpz_param_l);
pol commit instr_jmpz_pc_update_1;
instr_jmpz_pc_update_1 = ((1 - XIsZero) * (pc + 1));
...
pol commit pc_update;
pc_update = (((((instr_adjust_fp * label) + (instr__jump_to_operation * _operation_id)) + (instr__loop * pc)) + (instr_return * 0)) + ((1 - (((instr_adjust_fp + instr__jump_to_operation) + instr__loop) + instr_return)) * (pc + 1)));
pc' = ((1 - first_step') * pc_update);
pol constant p_line = [0, 1, 2, 3, 4] + [4]*;
pol constant p_instr__jump_to_operation = [0, 1, 0, 0, 0] + [0]*;
pol constant p_instr__loop = [0, 0, 0, 0, 1] + [1]*;
