...
pub fn bootloader_preamble() -> String {
    ...
    instr assert_bootloader_input X -> = bootloader_inputs.access X, val1 ->;
...
// Number of pages
val1 <== load_bootloader_input({NUM_PAGES_INDEX});
add_new_2 0;
set_reg 1, val3;

// Initialize memory hash
val1 <== load_bootloader_input({MEMORY_HASH_START_INDEX});
set_reg 18, val1;
val1  <== load_bootloader_input({MEMORY_HASH_START_INDEX} + 1);
set_reg 19, val1;
val1 <== load_bootloader_input({MEMORY_HASH_START_INDEX} + 2);
set_reg 20, val1;
val1 <== load_bootloader_input({MEMORY_HASH_START_INDEX} + 3);
set_reg 21, val1;
...
val1 <== get_reg(1);
val2 <== get_reg(0);
branch_if_zero bootloader_end_page_loop;

bootloader_start_page_loop:

// Page number
val2 <== get_reg(2);
val1 <== load_bootloader_input(val2 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET});
val2 <== get_reg(0);
and {PAGE_NUMBER_MASK};
set_reg 3, val3;
...
"#,
    ...
    bootloader.push_str("val3 <== get_reg(2);\n");
    bootloader.push_str("val1 <== get_reg(3);\n");
    bootloader.push_str(&format!("val1 <=X= val1 * {PAGE_SIZE_BYTES};\n"));
    for i in 0..WORDS_PER_PAGE {
        let reg = EXTRA_REGISTERS[(i % 4) + 4];
        bootloader.push_str(&format!(
            r#"
{reg} <== load_bootloader_input(val3 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {i});
val2 <=X= {reg};
mstore_bootloader {i} * {BYTES_PER_WORD};"#
        ));
...
branch_if_nonzero bootloader_level_{i}_is_right;
val1 <== get_reg(2);
{P4} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 0);
{P5} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 1);
{P6} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 2);
{P7} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 3);
...
{P7} <=X= {P3};
val1 <== get_reg(2);
{P0} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 0);
{P1} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 1);
{P2} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 2);
{P3} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 4 + {i} * 4 + 3);
...
val1 <== get_reg(9);
val2 <== get_reg(0);
branch_if_nonzero bootloader_update_memory_hash;

// Assert Correct Merkle Root
val2 <== get_reg(0);
val1 <== get_reg(18);
val1 <=X= {P0} - val1;
branch_if_nonzero bootloader_memory_hash_mismatch;
val1 <== get_reg(19);
val1 <=X= {P1} - val1;
branch_if_nonzero bootloader_memory_hash_mismatch;
val1 <== get_reg(20);
...
val1 <== get_reg(2);
{P0} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 0);
{P1} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 1);
{P2} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 2);
{P3} <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + 1 + {WORDS_PER_PAGE} + 3);
...
val1 <== get_reg(2);
val2 <== get_reg(1);
branch_if_nonzero bootloader_start_page_loop;

bootloader_end_page_loop:

// Assert final Merkle root is as claimed
val1 <== get_reg(18);
assert_bootloader_input {MEMORY_HASH_START_INDEX} + 4;
val1 <== get_reg(19);
assert_bootloader_input {MEMORY_HASH_START_INDEX} + 5;
val1 <== get_reg(20);
assert_bootloader_input {MEMORY_HASH_START_INDEX} + 6;
val1 <== get_reg(21);
assert_bootloader_input {MEMORY_HASH_START_INDEX} + 7;
...
val1 <== get_reg(1);
val2 <== get_reg(0);
branch_if_zero shutdown_end_page_loop;

shutdown_start_page_loop:

// Page number
val1 <== get_reg(2);
val1 <== load_bootloader_input(val1 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET});
val2 <== get_reg(0);
and {PAGE_NUMBER_MASK};
set_reg 3, val3;
...
"#,
    ...
    for i in 0..WORDS_PER_PAGE {
        let reg = EXTRA_REGISTERS[(i % 4) + 4];
        bootloader.push_str(&format!("mload {i} * {BYTES_PER_WORD};\n"));
        bootloader.push_str(&format!("{reg} <=X= val3;\n"));
...
val2 <== get_reg(2);
val1 <=X= {P0};
assert_bootloader_input val2 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + {WORDS_PER_PAGE} + 1 + 0;
val1 <=X= {P1};
assert_bootloader_input val2 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + {WORDS_PER_PAGE} + 1 + 1;
val1 <=X= {P2};
assert_bootloader_input val2 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + {WORDS_PER_PAGE} + 1 + 2;
val1 <=X= {P3};
assert_bootloader_input val2 * {BOOTLOADER_INPUTS_PER_PAGE} + {PAGE_INPUTS_OFFSET} + {WORDS_PER_PAGE} + 1 + 3;
...
val1 <== get_reg(2);
val2 <== get_reg(1);
branch_if_nonzero shutdown_start_page_loop;
...
pub const REGISTER_NAMES: [&str; 1] = ["main.pc"];
