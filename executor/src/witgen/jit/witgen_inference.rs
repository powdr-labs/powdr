...
impl<'a, T: FieldElement, FixedEval: FixedEvaluator<T>> WitgenInference<'a, T, FixedEval> {
    ...
    fn process_equality_on_row(
        ...
        lhs: &Expression<T>,
        offset: i32,
        rhs: &VariableOrValue<T, Variable<T>>,
    ...
    fn process_call(
        ...
        let vars = arguments
            ...
            .collect_vec();
        effects.push(Effect::MachineCall(bus_id, known, vars.clone()));
        ProcessResult {
            effects,
            complete: true,
        }
    ...
    fn process_assignments(&mut self) -> Result<Vec<Variable<T>>, Error> {
...
mod test {
    ...
    fn solve_on_rows(input: &str, rows: &[i32], known_cells: Vec<(&str, i32)>) -> String {
        ...
        let fixed_lookup_receives = fixed_data
            .bus_receives
            .values()
            .filter(|r| FixedLookup::is_responsible(r))
            .map(|r| (r.bus_id, r))
            .collect();
