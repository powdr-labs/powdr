...
impl<T: FieldElement> VMConverter<T> {
    ...
    fn convert_machine(mut self, mut input: Machine, rom: Option<Rom>) -> Machine {
        ...
        self.pil.push(PilStatement::PolynomialConstantDefinition(
            ...
            FunctionDefinition::Array(
                ArrayExpression::value(vec![Expression::Number(
                    SourceRef::unknown(),
                    Number {
                        value: 1u32.into(),
                        type_: None,
                ...
                |(mut value, mut instruction_literal_arg), (input, a)| {
                    match input {
                        ...
                        Input::Literal(_, LiteralKind::UnsignedConstant) => {
                            ...
                            if let Expression::Number(_, Number {value, type_: _}) = a {
                        ...
                        Input::Literal(_, LiteralKind::SignedConstant) => {
                            ...
                            } else if let Expression::UnaryOperation(_, UnaryOperation{op:UnaryOperator::Minus, e: expr}) = a
                            {
                                if let Expression::Number(_, Number {value, type_: _}) = *expr {
                                    instruction_literal_arg.push(InstructionLiteralArg::Number(
                                        -T::checked_from(value).unwrap(),
                                    ))
                                } else {
    ...
    fn process_assignment_value(&self, value: Expression) -> Vec<(T, AffineExpressionComponent)> {
        match value {
            ...
            Expression::Number(_, Number { value, type_: _ }) => {
            ...
            Expression::UnaryOperation(_, UnaryOperation { op, e }) => {
                ...
                self.negate_assignment_value(self.process_assignment_value(*e))
            }
        }
    }
                    ...
                    FunctionDefinition::Expression(Expression::LambdaExpression(
                        ...
                        LambdaExpression {
                            ...
                            body: Box::new(Expression::MatchExpression(
                                SourceRef::unknown(),
                                MatchExpression {
                                    e: Box::new(Expression::FunctionCall(
                                        SourceRef::unknown(),
                                        FunctionCall {
