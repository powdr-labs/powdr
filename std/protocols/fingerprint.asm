...
let<T: Add + FromLiteral> fingerprint: T[], Fp2<T> -> Fp2<T> = |expr_array, alpha| add_ext(alpha, fold(
    expr_array,
    from_base(0),
    |sum_acc, el| add_ext(sum_acc, from_base(el))
));

/// Maps [id, x_1, x_2, ..., x_n] to its Read-Solomon fingerprint, using a challenge alpha: $\sum_{i=1}^n alpha**{(n - i)} * x_i$
let<T: Add + FromLiteral> fingerprint_with_id: T, T[], Fp2<T> -> Fp2<T> = |id, expr_array, alpha| fingerprint([id] + expr_array, alpha);