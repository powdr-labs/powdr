...
// Evaluate one bus interaction for one APC row and update the matching
// periphery histogram. Short-circuits on multiplicity == 0.
__device__ __forceinline__ void apply_one_interaction(
  DevInteraction intr,
  size_t r,
  const uint32_t* __restrict__ d_bytecode,
  const Fp* __restrict__ d_output,
  const ExprSpan* __restrict__ d_arg_spans,
...
__global__ void apc_apply_bus_kernel(
  ...
  for (size_t i = 0; i < n_interactions; ++i) {
    apply_one_interaction(
      d_interactions[i], (size_t)r,
      d_bytecode, d_output, d_arg_spans,
