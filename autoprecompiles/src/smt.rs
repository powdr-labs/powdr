...
use crate::{SymbolicConstraint, SymbolicMachine};
...
fn algebraic_to_smt<T: FieldElement>(expr: &AlgebraicExpression<T>) -> String {
    match expr {
...
// }
//
// fn symbolic_bus_interaction_to_smt<T: FieldElement>(
//     b: &SymbolicBusInteraction<T>,
// ) -> SmtConstraints {
//     SmtConstraints::from_range_constraints(if b.id == 3 {
...
//     let v = algebraic_to_smt(&b.args[0]);
...
//     assert_eq!(b.id, 7);
//     assert_eq!(b.args.len(), 2);
//     let v1 = algebraic_to_smt(&b.args[0]);
//     let v2 = algebraic_to_smt(&b.args[1]);
...
//     let v1 = algebraic_to_smt(&b.args[0]);
//     let v2 = algebraic_to_smt(&b.args[1]);
//     let v3 = algebraic_to_smt(&b.args[2]);
//     let v4 = algebraic_to_smt(&b.args[3]);
//     vec![
...
fn symbolic_machine_to_smtlib2<T: FieldElement>(machine: &SymbolicMachine<T>) -> String {
    ...
    for poly in &machine.constraints {
        let c_smt = algebraic_to_smt(&poly.expr);
...
fn negate_constraints_smtlib2<T: FieldElement>(constraints: Vec<SymbolicConstraint<T>>) -> String {
    let inner = constraints
        .iter()
        .map(|poly| format!("(= (as ff0 BB) {})", algebraic_to_smt(&poly.expr)))
        .collect::<Vec<_>>()
        .join(" ");
...
pub fn detect_redundant_constraints<T: FieldElement>(
    machine: &SymbolicMachine<T>,
) -> Vec<Vec<usize>> {
    println!("machine:\n{machine}");
