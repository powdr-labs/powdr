...
impl<F: PrimeField32> VmExtension<F> for HintsExtension {
    ...
    fn build(
        ...
        builder.add_phantom_sub_executor(K256InverseFieldSubEx, PhantomDiscriminant(HintsPhantom::HintK256InverseField as u16))?;
        builder.add_phantom_sub_executor(K256InverseField10x26SubEx, PhantomDiscriminant(HintsPhantom::HintK256InverseField10x26 as u16))?;
        builder.add_phantom_sub_executor(K256SqrtField10x26SubEx, PhantomDiscriminant(HintsPhantom::HintK256SqrtField10x26 as u16))?;
        Ok(inventory)
    }
}
...
impl_modulus!(K256Mod, U256, "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFC2F");
...
impl<F: PrimeField32> PhantomSubExecutor<F> for K256InverseFieldSubEx {
    fn phantom_execute(
        ...
        let bytes: [u8;32] = memory.unsafe_read::<32>(
            F::from_canonical_u32(RV32_MEMORY_AS),
            F::from_canonical_u32(rs1),
        )
            .into_iter()
        ...
        let inv_bytes = n_inv.to_be_bytes().into_iter().map(|b| F::from_canonical_u8(b)).collect();
...
impl<F: PrimeField32> PhantomSubExecutor<F> for K256InverseField10x26SubEx {
    fn phantom_execute(
        ...
        let bytes: [u8;40] = memory.unsafe_read::<40>(
            F::from_canonical_u32(RV32_MEMORY_AS),
            F::from_canonical_u32(rs1),
        )
            .into_iter()
...
impl<F: PrimeField32> PhantomSubExecutor<F> for K256SqrtField10x26SubEx {
    fn phantom_execute(
        ...
        if res.is_some().into() {
            ...
            streams.hint_stream = 1u32.to_le_bytes() // indicates that a square root exists
                .into_iter()
            ...
            streams.hint_stream = 0u32.to_le_bytes()
                .map(|b| F::from_canonical_u8(b))
                .into_iter().collect();
        }
