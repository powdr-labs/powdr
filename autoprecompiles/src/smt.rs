...
fn range_check_interaction_to_smt<T: FieldElement>(b: &SymbolicBusInteraction<T>) -> Vec<String> {
    assert_eq!(b.id, 3);
...
fn range_check_to_smt(v: String, max_range: u64) -> String {
    assert!(max_range <= 256);
    let terms = (0..max_range)
        .map(|i| format!("(ff.add {v} (ff.neg (as ff{i} BB)))"))
        .collect::<Vec<_>>();
...
fn symbolic_machine_to_smtlib2_ff<T: FieldElement>(
    ...
    for bus_int in &machine.bus_interactions {
        ...
        for bus_constraint in bus_int_smt {
            let a = format!("(assert (= (as ff0 BB) {bus_constraint}))");
            smt2.push_str(&a);
            smt2.push('\n');
        }
    }
