...
use super::{
    ...
    symbolic_expression::{
        BinaryOperator, BitOperator, RPNExpression, RPNExpressionElem, SymbolicExpression,
        UnaryOperator,
    },
...
pub fn interpret_effects<T: FieldElement>(
    ...
    Ok(WitgenFunction::Interpreted(EffectsInterpreter::new(
        first_column_id,
        column_count,
        known_inputs,
        effects,
...
impl<T: FieldElement> EffectsInterpreter<T> {
    ...
    fn call<Q: QueryCallback<T>>(&self, params: WitgenFunctionParams<'_, T>) {
...
impl<T> MutSlice<T> {
    #[inline]
    fn into_mut_slice<'a>(self) -> &'a mut [T] {
