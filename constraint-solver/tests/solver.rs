...
fn byte_decomposition() {
    ...
    let solver = Solver::new(constraint_system)
        .with_bus_interaction_handler(Arc::new(TestBusInteractionHandler {}));
...
fn one_hot_flags() {
    let constraint_system = ConstraintSystem {
        algebraic_constraints: vec![
            ...
            // Flag 2 is active
            var("flag0") * constant(12)
                + var("flag1") * constant(123)
                + var("flag2") * constant(1234)
                + var("flag3") * constant(12345)
                - constant(1234),
        ],
        bus_interactions: vec![],
    };
