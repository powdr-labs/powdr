                ...
                && rc.range().0.is_zero()
            {
                ...
                return Ok(ProcessResult {
                    effects: self
                        ...
                        .iter()
                        .map(|(var, coeff)| {
                            let value = if coeff.try_to_number().map(|n| n.is_in_lower_half())
                                == Some(true)
                            {
                                range_constraints.get(var).range().0
                            } else {
                                range_constraints.get(var).range().1
                            };
                            Effect::Assignment(var.clone(), T::from(value))
                        })
                        .collect(),
                    complete: true,
                });
