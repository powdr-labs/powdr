...
impl<'a, T: FieldElement> Machine<'a, T> for FixedLookup<'a, T> {
    ...
    fn can_process_call_fully(
        ...
        let input_range_constraints = known_arguments
            ...
            .filter_map(|(is_known, range_constraint)| {
                is_known.then_some(range_constraint.as_ref())
            })
            .collect_vec();

        // Now see that the matches are all unique and determine range constraints.
        ...
        let Some(first) = values.next() else {
            ...
            return Some(vec![None; range_constraints.len()]);
        };
        // If the first item is None, it means the match was not unique.
...
fn matches_range_constraint<T: FieldElement>(
    ...
    values
        .iter()
        .zip_eq(range_constraints)
        .all(|(value, range_constraint)| {
            range_constraint
                .as_ref()
                .map(|range_constraint| range_constraint.allows_value(*value))
                .unwrap_or(true)
        })
}
