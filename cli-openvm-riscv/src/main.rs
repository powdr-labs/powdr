...
struct GenerateApcsArgs {
    ...
    /// Disable static superblocks: sequences of basic blocks linked by statically determined jumps.
    /// Static superblocks are enabled by default.
    #[arg(long, default_value_t = false)]
    no_static_superblocks: bool,
}
...
fn validate_select_args(args: &SelectArgs, for_execution: bool) {
    if args.generate.optimistic_superblocks > 1 && !matches!(args.pgo, PgoType::Cell) {
        Cli::command()
            .error(
    ...
    if for_execution && args.generate.optimistic_superblocks > 1 {
        Cli::command()
            .error(
                clap::error::ErrorKind::ArgumentConflict,
