...
use powdr_openvm_riscv::{compile_openvm, default_powdr_openvm_config, CompiledProgram, GuestOptions};
use powdr_openvm_riscv::{detect_empirical_constraints, OriginalCompiledProgram};
...
#[derive(Parser)]
#[command(name = "powdr-openvm-riscv", author, version, about, long_about = None)]
struct Cli {
    #[command(subcommand)]
...
fn run_command(command: Commands) {
    ...
    match command {
        Commands::Compile { guest, shared } => {
            ...
            let execution_profile = powdr_openvm_riscv::execution_profile_from_guest(
                &guest_program,
        ...
        Commands::Execute {
            ...
            let compile_and_exec = || {
                ...
                .unwrap();
                powdr_openvm_riscv::execute(program, stdin_from(shared.input)).unwrap();
            };
