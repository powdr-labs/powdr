...
use crate::powdr_extension::{OriginalInstruction, PowdrOpcode, PowdrPrecompile};
use crate::OpenVmField;
use crate::{instructions_to_airs, CompiledProgram, OriginalCompiledProgram, SpecializedConfig};
use crate::{pgo, IntoOpenVm, PgoMode};
...
use crate::{powdr_extension::PowdrExtension, utils::symbolic_to_algebraic};
...
type CachedAutoPrecompile<P> = (
    usize,              // powdr opcode
    SymbolicMachine<P>, // autoprecompile
    ...
    Val<SC>: PrimeField32,
{
    pub fn new(
        ...
        let used_instructions = program
            .exe
            .program
            .instructions_and_debug_infos
            .iter()
            .filter_map(|x| x.as_ref().map(|x| x.0.opcode));

        let airs =
            instructions_to_airs::<SC, _, _>(pcs, program.sdk_vm_config.clone(), used_instructions);
    ...
    fn pgo(mut self) -> Self {
        ...
        match pgo_mode {
            PgoMode::Cell => {
                tracing::info!("PGO mode: Cell");
                self.sort_blocks_by_pgo_cell_cost();
            }
            PgoMode::Instruction => {
                tracing::info!("PGO mode: Instruction");
                self.sort_blocks_by_pgo_instruction_cost();
            }
            PgoMode::None => {
                tracing::info!("PGO mode: None");
                self.sort_blocks_by_length();
            }
        }
        self
    }

    fn sort_blocks_by_pgo_instruction_cost(&mut self) {
        let pgo_program_idx_count =
            pgo::<SC, P, E>(&self.program, self.stdin.clone().unwrap()).unwrap();
        // drop any block whose start index cannot be found in pc_idx_count,
        // because a basic block might not be executed at all.
        // Also only keep basic blocks with more than one original instruction.
        self.blocks
            .retain(|b| pgo_program_idx_count.contains_key(&(b.start_idx as u32)));
        ...
        self.blocks.sort_by(|a, b| {
            ...
            (b_cnt * (b.len() as u32)).cmp(&(a_cnt * (a.len() as u32)))
        });
        ...
        for block in &self.blocks {
            ...
            let cost = count * (block.len() as u32);
    ...
    fn sort_blocks_by_length(&mut self) {
        // cost = number_of_original_instructions
        self.blocks.sort_by_key(|b| std::cmp::Reverse(b.len()));
        // Debug print blocks by descending cost
        for block in &self.blocks {
            let start_idx = block.start_idx;
            tracing::info!(
    ...
    fn sort_blocks_by_pgo_cell_cost(&mut self) {
        let pgo_program_idx_count =
            pgo::<SC, P, E>(&self.program, self.stdin.clone().unwrap()).unwrap();
        ...
        let air_width_by_opcode = self
            .airs
            .iter()
            .filter(|&(i, _)| (!self.opcodes_no_apc.contains(i)))
        ...
        let cells_saved_per_row_by_bb: HashMap<_, _> = self
            ...
            .filter_map(|acc_block| {
                ...
                let original_cells_per_row: usize =
                    self.program.exe.program.instructions_and_debug_infos
                        [acc_block.start_idx..acc_block.start_idx + acc_block.len]
                        .iter()
                        .map(|instr| instr.as_ref().unwrap().0.opcode)
                        .map(|opcode| air_width_by_opcode.get(&opcode.as_usize()).unwrap())
                        .sum();
    ...
    fn build(mut self) -> CompiledProgram<P> {
        ...
        let n_acc = self.config.autoprecompiles as usize;
        ...
        let vm_config = SpecializedConfig::from_base_and_extension(
            self.program.sdk_vm_config.clone(),
            PowdrExtension::new(
                extensions,
                self.program.sdk_vm_config,
...
impl<P: IntoOpenVm> BasicBlock<P> {
    pub fn apc(
        &mut self,
        airs: &BTreeMap<usize, SymbolicMachine<P>>,
        ...
        self.apc
            .as_ref()
            .expect("APC should be built after calling `build`")
    }
    ...
    fn pretty_print(
        &self,
        program: &VmExe<P::Field>,
        instr_formatter: impl Fn(&Instruction<P::Field>) -> String,
    ) -> String {
        format!(
            "BasicBlock(start_idx: {}, len: {}, apc: {:?}, instructions: [\n{}])",
            self.start_idx,
            self.len,
            self.apc.as_ref().map(|(opcode, _, _)| opcode),
            program.program.instructions_and_debug_infos[self.start_idx..self.start_idx + self.len]
                .iter()
...
pub fn collect_basic_blocks<P: IntoOpenVm>(
    program: &Program<P::Field>,
    labels: &BTreeSet<u32>,
    opcodes_no_apc: &[usize],
    branch_opcodes: &BTreeSet<usize>,
) -> Vec<BasicBlock<P>> {
    let mut blocks = Vec::new();
    let mut curr_block = BasicBlock::default();
...
fn generate_autoprecompile<P: IntoOpenVm>(
    ...
    let program = exe.program.instructions_and_debug_infos
        [block.start_idx..block.start_idx + block.len]
        .iter()
        .map(|i| &i.as_ref().unwrap().0)
