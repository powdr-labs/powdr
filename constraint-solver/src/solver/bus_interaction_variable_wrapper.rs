...
pub enum Variable<V> {
    Variable(V),
    BusInteractionField(BusInteractionField),
}

#[derive(Clone, Debug, Ord, PartialOrd, Eq, PartialEq, Hash)]
pub struct BusInteractionField {
    interaction: usize,
    field: usize,
}

pub fn to_system_with_bus_interaction_variables<
    T: FieldElement,
...
>(
    ...
    BTreeMap<BusInteractionField, QuadraticSymbolicExpression<T, V>>,
) {
        ...
        .map(|(bus_interaction_index, bus_interaction)| {
            BusInteraction::from_iter(bus_interaction.fields().enumerate().map(
                |(field_index, expr)| {
                    ...
                    let field = BusInteractionField {
                        interaction: bus_interaction_index,
                        field: field_index,
                    };
                    let v = Variable::BusInteractionField(field.clone());
                    new_constraints.push(
                        transformed_expr
                            - QuadraticSymbolicExpression::from_unknown_variable(v.clone()),
                    );
                    bus_interaction_vars.insert(field, expr.clone());
...
impl<V: Display> Display for Variable<V> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Variable::BusInteractionField(BusInteractionField { interaction, field }) => {
                write!(f, "BusInteractionField({interaction}, {field})")
            }
        }
    }
}
